...
export default function Home() {
  ...
  // ---------- design the set ----------
  async function handleDesign(briefText: string, targetLengthMinutes: number | undefined, mcMode: boolean) {
    setDesigning(true);
    setBrief(briefText);
    try {
      const r = await api.plan(briefText, tracks, targetLengthMinutes, mcMode);
  ...
  async function handleSkip() {
    await api.next();
    void steer("The DJ pressed skip on the current track — treat this as feedback and adjust the upcoming energy arc if it makes sense.", true);
  }
  ...
  async function steer(instruction: string, isAutoFeedback = false) {
    ...
    try {
      const res = await api.steer({
        ...
        remainingSetlist: remainingRef.current,
        playedHistory: playedHistoryRef.current,
        bannedUris: bannedUrisRef.current,
      });
          ...
          {remaining.length === 0 ? (
            ...
            <>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-lg font-semibold">The set</h2>
        ...
        <div className="space-y-5">
          {showCrossfadeTip && (
            <div className="flex items-center justify-between rounded-lg border border-booth-accent2/30 bg-booth-accent2/10 px-3 py-2 text-xs text-booth-accent2">
              <span>
                Tip: enable Spotify&rsquo;s built-in crossfade (Settings → Playback) for smoother transitions — this
                app controls order and queue, not the audio itself.
              </span>
